...
const I = {
  ...
  ArrowR:  ()=><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>,
}
  ...
  return (
    <div style={{ maxWidth:1140 }}>
          ...
          <motion.div whileHover={{ y:-3,boxShadow:'0 12px 32px rgba(124,58,237,.15)' }}
            ...
            <div>
              <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:12 }}>
                <div style={{ width:42,height:42,borderRadius:12,background:'rgba(255,255,255,.18)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',flexShrink:0 }}>
                  <I.Compass/>
                </div>
                <div>
                  ...
                  <h2 style={{ margin:0,fontSize:'1.1rem',fontWeight:800,color:'white',lineHeight:1.2 }}>Find My Topic</h2>
                </div>
              </div>
              <p style={{ margin:0,fontSize:'.84rem',color:'rgba(255,255,255,.85)',lineHeight:1.65 }}>
                Not sure what to research? Answer 8 quick questions and our AI will suggest 10–15 personalised, ranked topics — then help you refine the best one.
              </p>
            </div>
            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:18 }}>
              <div style={{ display:'flex',gap:6 }}>
                {['Confused?','Need ideas','Starting fresh'].map(tag=>(
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:20 }}>
        {statCards.map((c,i) => (
          <motion.div key={c.label} {...fu(.2+i*.05)} className="g-card" style={{ padding:'18px 20px',position:'relative',overflow:'hidden' }}>
            ...
            <div style={{ width:32,height:32,borderRadius:9,background:c.bg,border:`1px solid ${c.accent}18`,display:'flex',alignItems:'center',justifyContent:'center',color:c.accent,marginBottom:12 }}>{c.icon}</div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 308px',gap:18,alignItems:'start' }}>
        <motion.div {...fu(.36)}>
          <div className="g-card-flat" style={{ overflow:'hidden' }}>
            ...
            {pL ? (
              ...
              <div style={{ padding:'40px 20px',textAlign:'center' }}>
                ...
                <p style={{ fontSize:'.84rem',color:'#9ca3af',marginBottom:18 }}>Start with "Find My Topic" if you need help, or jump straight to generating your spec.</p>
                <div style={{ display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap' }}>
                  <Link href="/dashboard/topics" style={{ textDecoration:'none' }}>
                    <button className="g-btn-ghost" style={{ fontSize:'.83rem' }}>Find My Topic</button>
                  </Link>
                  <Link href="/dashboard/generate" style={{ textDecoration:'none' }}>
              ...
              <motion.div key={p.id} initial={{opacity:0,x:-8}} animate={{opacity:1,x:0}} transition={{delay:.36+i*.05}}
                ...
                <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ display:'flex',alignItems:'center',gap:7 }}>
                    ...
                    <span style={{ fontSize:'.68rem',color:'#9ca3af' }}>{p.field_of_study} · {ago(p.created_at)}</span>
                  </div>
                </div>
          ...
          {!pL&&recent.length>0&&(
            <motion.div {...fu(.48)} style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginTop:12 }}>
        ...
        <div style={{ display:'flex',flexDirection:'column',gap:14 }}>
          {avgScore>0 && (
            <motion.div {...fu(.4)} className="g-card" style={{ padding:'20px',textAlign:'center' }}>
          ...
          <motion.div {...fu(.44)} className="g-card-flat" style={{ overflow:'hidden' }}>
            ...
            <div style={{ padding:'8px 8px',display:'flex',flexDirection:'column',gap:3 }}>
              {[
                {icon:<I.Compass/>, label:'Find my topic',   sub:'AI topic discovery',  href:'/dashboard/topics',   c:'#7c3aed',bg:'#faf5ff'},
          ...
          <motion.div {...fu(.5)} style={{ background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:14,padding:'18px',position:'relative',overflow:'hidden' }}>
            ...
            <p style={{ margin:0,fontSize:'.82rem',color:'#374151',lineHeight:1.65 }}>
              ...
              :<>New to research? Start with <strong style={{color:'#7c3aed'}}>"Find My Topic"</strong> — our AI will guide you from confusion to a clear, viable research direction.</>}
            </p>
          </motion.div>
        </div>
      </div>
