...
interface ScoutData { scout_type:string; datasets:ScoutDataset[]; papers:ScoutPaper[]; tools:ScoutTool[]; key_authors:ScoutKeyAuthor[]; availability_summary:string; data_verdict:string; advisor_context:string }
...
// Stage type — 'results' is reachable from 'chat' without restarting
...
function renderMessage(text: string, isUser = false) {
  ...
  while (i < lines.length) {
    ...
    if (trimmed.startsWith('>>>') && trimmed.includes('<<<')) {
      ...
      elements.push(
        <div key={`q-${i}`} style={{ margin:'14px 0', padding:'14px 16px', borderRadius:12, background:'#faf5ff', border:'2px solid #7c3aed', position:'relative' }}>
          <div style={{ position:'absolute', top:-10, left:12, background:'#7c3aed', color:'white', fontSize:'.6rem', fontWeight:800, padding:'2px 9px', borderRadius:999, letterSpacing:'.08em', textTransform:'uppercase' }}>
            Important — Please Answer This
          </div>
    ...
    if (/^\d+\.\s/.test(trimmed)) {
      ...
      elements.push(
        <ol key={`ol-${i}`} style={{ margin:'8px 0', paddingLeft:20, display:'flex', flexDirection:'column', gap:5 }}>
    ...
    // Section label: "Label:" — plain line ending with colon
    if (/^[A-Z][A-Za-z\s]{2,50}:$/.test(trimmed)) {
      elements.push(
        <div key={`lbl-${i}`} style={{ fontWeight:700, color:'#0f1f0f', fontSize:'.83rem', marginTop:14, marginBottom:4, paddingTop:10, borderTop:'1px solid rgba(0,0,0,.07)' }}>
          {trimmed.replace(/:$/, '')}
        </div>
      )
      i++; continue
    ...
    // Normal paragraph
    elements.push(<p key={`p-${i}`} style={{ margin:'0 0 7px', fontSize:'.86rem', lineHeight:1.7, color:'#374151' }}>{trimmed}</p>)
    i++
  }
...
function StageBar({ current }: { current: number }) {
  const steps = [
    { n:1, label:'Profile', icon:I.Brain },
    { n:2, label:'Topics',  icon:I.Compass },
    { n:3, label:'AI Chat', icon:I.Chat },
    { n:4, label:'Done',    icon:I.Flag },
  ]
  return (
      ...
      {steps.map(s => {
        ...
        return (
          <div key={s.n} style={{ display:'flex', flexDirection:'column', alignItems:'center', zIndex:1, flex:1 }}>
            ...
            <p style={{ margin:'7px 0 0', fontSize:'.71rem', fontWeight:active?700:500,
              color:done?'#16a34a':active?'#7c3aed':'#9ca3af', textAlign:'center' }}>{s.label}</p>
          </div>
        )
      })}
    </div>
      ...
      {options.map(o => {
        ...
        return (
          <button key={o} onClick={() => onToggle(o)}
            style={{ padding:'8px 14px', borderRadius:999, border:`1.5px solid ${on?border:'#e8ede8'}`,
              background:on?bg:'white', color:on?color:'#6b7280',
              fontSize:'.82rem', fontWeight:on?700:500, cursor:'pointer', transition:'all .15s' }}>
            {on && '✓ '}{o}
          </button>
        )
      })}
      ...
      {options.map(o => {
        ...
        return (
          <button key={o.value} onClick={() => onSelect(o.value)}
            style={{ padding:'12px 14px', borderRadius:12, border:`2px solid ${on?border:'#e8ede8'}`,
              background:on?bg:'white', cursor:'pointer', transition:'all .15s', textAlign:'left' }}>
...
const Q_OPTIONS: Record<string, any[]> = {
  degree_level: [
    { value:'BSc', label:"Bachelor's (BSc)", sub:'Undergraduate' },
    { value:'MSc', label:"Master's (MSc)",   sub:'Postgraduate' },
  ],
  project_type: [
    { value:'research-based', label:'Research-based', sub:'Literature & theory' },
    { value:'practical',      label:'Practical',      sub:'Build / implement' },
    { value:'mixed',          label:'Mixed',           sub:'Both' },
  ...
  geographic_focus: [
    { value:'university', label:'My University', sub:'Campus context' },
    { value:'city',       label:'City / Region', sub:'Urban focus' },
    { value:'country',    label:'My Country',    sub:'National' },
    { value:'africa',     label:'Africa',         sub:'Pan-African' },
    { value:'europe',     label:'Europe',         sub:'European' },
    { value:'global',     label:'Global',         sub:'Worldwide' },
    { value:'none',       label:'No focus',       sub:'Topic-agnostic' },
  ],
  ambition_level: [
    { value:'manageable',  label:'Manageable',  sub:'Safe, low stress' },
    { value:'impressive',  label:'Impressive',  sub:'Strong novel angle' },
    { value:'distinction', label:'Distinction', sub:'Top grade target' },
    { value:'cv-strong',   label:'CV-Strong',   sub:'Industry-ready' },
  ],
  confidence_level: [
    { value:'very-confused',   label:'Very Confused',   sub:'No idea where to start' },
    { value:'somewhat-unsure', label:'Somewhat Unsure', sub:'Area known, no topic' },
    { value:'rough-direction', label:'Rough Direction', sub:'Need narrowing' },
    { value:'have-idea',       label:'Have an Idea',    sub:'Need validation' },
  ],
}

// Questions that use radio cards (single select)
const RADIO_IDS = ['degree_level','project_type','geographic_focus','ambition_level','confidence_level']

/* ══════════════════════════════════════════════════════════════════════════
   FIX 2 — FormStage
   CHANGE: Removed auto-advance on radio select.
   CHANGE: Every slide always shows BOTH Back and Continue buttons.
   CHANGE: Clicking a radio card just selects it — Continue advances.
   This way one click = select, second click (Continue) = advance.
...
function FormStage({ onSubmit }: { onSubmit:(d:FormData)=>void }) {
  ...
  const canNext = () => {
    const v = form[QUESTIONS[q].id as keyof FormData]
  ...
  const back = () => setQ(p => Math.max(0, p-1))
  ...
  const renderQuestion = () => {
    ...
    if (qid === 'preferred_activity' || qid === 'interest_areas') {
      const isInt = qid === 'interest_areas'
      return (
        <ChipSelect options={Q_OPTIONS[qid] as string[]} selected={form[qid] as string[]}
          onToggle={v => toggleArr(qid as any, v)}
          color={isInt?'#16a34a':'#7c3aed'} bg={isInt?'#f0fdf4':'#faf5ff'} border={isInt?'#bbf7d0':'#e9d5ff'}/>
      )
    }

    // Radio cards — NOTE: onSelect only selects, does NOT auto-advance
    const isGreen = ['ambition_level','confidence_level'].includes(qid)
    return (
      <RadioCards options={Q_OPTIONS[qid]} selected={form[qid as keyof FormData] as string}
        onSelect={v => update(qid as keyof FormData, v)}
        color={isGreen?'#16a34a':'#7c3aed'} bg={isGreen?'#f0fdf4':'#faf5ff'} border={isGreen?'#bbf7d0':'#e9d5ff'}/>
    )
  }
  ...
  return (
    <div style={{ maxWidth:640, margin:'0 auto' }}>
      ...
      {/* FIX 2: Both buttons on EVERY slide, no exceptions */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:28 }}>
        <button onClick={back} disabled={q===0} className="g-btn-outline"
          style={{ opacity:q===0?0.3:1, display:'flex', alignItems:'center', gap:6 }}>
          <I.Back/> Back
        </button>
        <button onClick={next} disabled={!canNext()} className="g-btn"
          style={{ opacity:canNext()?1:0.4, display:'flex', alignItems:'center', gap:6 }}>
...
function LoadingStage() {
  const steps = ['Analysing your profile…','Scanning your field…','Matching interests to gaps…','Ranking by suitability…','Grouping clusters…','Finalising your list…']
...
function ScoutingStage({ topic, isLiterature }: { topic:Topic; isLiterature:boolean }) {
  return (
    <div style={{ maxWidth:480, margin:'80px auto 0', textAlign:'center' }}>
      <style>{`@keyframes sp{to{transform:rotate(360deg)}} .sp{animation:sp 1s linear infinite;display:inline-block}`}</style>
      <div style={{ width:72, height:72, borderRadius:20, background:'linear-gradient(135deg,#16a34a,#22c55e)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 24px', boxShadow:'0 8px 32px rgba(22,163,74,.25)' }}>
        <div className="sp" style={{ color:'white' }}><I.Search/></div>
      </div>
      <h2 style={{ margin:'0 0 8px', fontWeight:800, color:'#0f1f0f', fontSize:'1.2rem', fontFamily:'Fraunces,serif' }}>
      ...
      <p style={{ margin:'0 0 20px', fontSize:'.86rem', color:'#6b7280', lineHeight:1.6 }}>
        Checking what exists for<br/>
        <strong style={{ color:'#0f1f0f' }}>"{topic.title.length>60 ? topic.title.slice(0,60)+'…' : topic.title}"</strong>
      </p>
...
/* ─── Results grid ───────────────────────────────────────────────────────── */
function ResultsStage({ result, onSelect }: {
  result: { clusters:string[]; topics:Topic[]; prompt_note:string }
    ...
    <div>
      ...
      <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:22 }}>
        {clusters.map(c => (
          <button key={c} onClick={() => setCluster(c)}
            ...
            {c}
            {c==='All' && <span style={{ marginLeft:5, background:cluster===c?'rgba(255,255,255,.25)':'#f0fdf4', color:cluster===c?'white':'#16a34a', borderRadius:999, padding:'0 6px', fontSize:'.68rem', fontWeight:700 }}>{result.topics.length}</span>}
          </button>
        ))}
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:14 }}>
        {filtered.map((topic, i) => (
          <motion.div key={topic.id} initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*.04 }}
    ...
    <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ duration:.4 }}
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <button onClick={() => setExpanded(e => !e)} style={{ background:'none', border:'none', cursor:'pointer', fontSize:'.72rem', color:'#16a34a', fontWeight:700 }}>
            ...
            <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:8, padding:'7px 10px', background:'white', borderRadius:8, marginBottom:5, border:'1px solid #bbf7d0' }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                {d.url && <a href={d.url} target="_blank" rel="noreferrer"
                  style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:'.73rem', color:'#7c3aed', fontWeight:700, textDecoration:'none', padding:'2px 8px', background:'#faf5ff', borderRadius:6, border:'1px solid #e9d5ff' }}>
                  <I.Link/> View Dataset
                </a>}
              </div>
            </div>
            ...
            <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:8, padding:'7px 10px', background:'white', borderRadius:8, marginBottom:5, border:'1px solid #e5e7eb' }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                {p.url && <a href={p.url} target="_blank" rel="noreferrer"
                  style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:'.72rem', color:'#2563eb', fontWeight:600, textDecoration:'none' }}>
                  <I.Link/> Open Paper
                </a>}
              </div>
            </div>
      ...
      {data.availability_summary && (
        <p style={{ margin:'8px 0 0', fontSize:'.76rem', color:'#374151', lineHeight:1.6, fontStyle:'italic' }}>{data.availability_summary}</p>
      )}
    </motion.div>
  )
}
...
/* ══════════════════════════════════════════════════════════════════════════
   ChatStage — all fixes applied
   FIX 1: useRef guard (no double call in Strict Mode)
   FIX 3: "Change Topic" button returns to results grid without restarting
   FIX 4: "Seek Professional Help" button — correct text "Dropping soon"
   FIX 1 (advisor): project_type + preferred_activity passed to refine
══════════════════════════════════════════════════════════════════════════ */
function ChatStage({ topic, formData, scoutData, onFinal, onChangeTopic }:
  ...
  const callAI = useCallback(async (stage: typeof chatStage, userMsg?: string) => {
    ...
    try {
      ...
      const res = await axios.post(`${API}/topics/refine`, {
        ...
        project_type:       formData.project_type,       // FIX 1 (advisor): context-aware questions
        preferred_activity: formData.preferred_activity, // FIX 1 (advisor): context-aware questions
        stage,
      ...
      setMessages(p => [...p, { role:'ai', content:"Hmm, something went sideways on my end. Give it another shot?" }])
  ...
  const sendMessage = async () => {
    ...
    if (chatStage==='explain' || chatStage==='questions') {
      if (newCount >= 4) { nextStage='feasibility'; setChatStage('feasibility') } else setChatStage('questions')
    } else if (chatStage==='feasibility') { nextStage='final'; setChatStage('final') } else nextStage='final'
  ...
  return (
    <div style={{ maxWidth:720, margin:'0 auto' }}>
      ...
      {/* Topic header with Change Topic button */}
        ...
        <div style={{ height:400, overflowY:'auto', padding:'18px' }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display:'flex', justifyContent:m.role==='user'?'flex-end':'flex-start', marginBottom:14 }}>
              ...
              <div style={{ maxWidth:'78%', padding:'12px 15px',
                borderRadius:m.role==='user'?'14px 14px 4px 14px':'14px 14px 14px 4px',
                background:m.role==='user'?'#7c3aed':'#f9fafb',
                border:m.role==='user'?'none':'1px solid #e8ede8',
                color:m.role==='user'?'white':'#374151', lineHeight:1.7 }}>
        ...
        {chatStage !== 'final' && (
          <div style={{ padding:'12px 14px', borderTop:'1px solid #f0f4f0', display:'flex', gap:8 }}>
            <input value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder="Type your response here…" disabled={loading}
      ...
      {/* Bottom bar: stage hint + Seek Professional Help */}
      <div style={{ marginTop:12, display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
        <p style={{ margin:0, fontSize:'.73rem', color:'#9ca3af', lineHeight:1.4 }}>
          {chatStage==='explain' ? 'Your advisor is introducing your topic…'
           : chatStage==='questions' ? `Feasibility check · ~${questionCount}/4 questions covered`
           : chatStage==='feasibility' ? 'Assessing your situation honestly…'
           : 'Writing your final refined topic…'}
        </p>

        {/* FIX 4: "Seek Professional Help" — correct text, always visible */}
        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
          {helpClicked && (
            <motion.span initial={{ opacity:0, x:8 }} animate={{ opacity:1, x:0 }}
              style={{ fontSize:'.78rem', color:'#9ca3af', fontStyle:'italic' }}>
              Dropping soon
            </motion.span>
          )}
          <button onClick={() => setHelpClicked(true)}
            style={{ display:'flex', alignItems:'center', gap:5, padding:'7px 13px', borderRadius:9, border:'1.5px solid #fecaca', background:'#fef2f2', color:'#dc2626', fontSize:'.76rem', fontWeight:700, cursor:'pointer', transition:'all .15s' }}
            onMouseEnter={e => { const el=e.currentTarget as HTMLElement; el.style.background='#fee2e2'; el.style.borderColor='#fca5a5' }}
            onMouseLeave={e => { const el=e.currentTarget as HTMLElement; el.style.background='#fef2f2'; el.style.borderColor='#fecaca' }}>
...
/* ══════════════════════════════════════════════════════════════════════════
   FinalStage
   FIX 5: "Use This Topic" autofills topic in spec generator via sessionStorage
   FIX 6: Clear note telling user to download similar projects and add manually
══════════════════════════════════════════════════════════════════════════ */
function FinalStage({ topic, description, field, degree, formData }: {
  topic:string; description:string; field:string; degree:string; formData:FormData
}) {
  ...
  // FIX 5: Store everything spec generator needs in sessionStorage
  const handleUseThis = () => {
    ...
    sessionStorage.setItem('prefill_project_type', formData.project_type)
    if (projects && projects.length>0) {
      sessionStorage.setItem('prefill_similar_projects', JSON.stringify(projects))
    }
  ...
  return (
    <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:.5 }} style={{ maxWidth:680, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-start', gap:12 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <p style={{ margin:'0 0 12px', fontSize:'.8rem', color:'#6b7280', lineHeight:1.55 }}>
              Our AI will search real student theses and dissertations similar to your topic — you can use them as reference material in spec generation.
            </p>
          ...
          <div style={{ marginTop:14 }}>
            ...
            {projects.length===0
              ? <div style={{ padding:'14px', background:'#f9fafb', borderRadius:10, textAlign:'center' }}>
                  <p style={{ margin:0, fontSize:'.82rem', color:'#9ca3af' }}>No similar projects found publicly. You can still proceed — skip the past projects step in the generator or upload your own.</p>
                </div>
              : <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  {projects.map((p, i) => (
                    <div key={i} style={{ background:'#f9fafb', border:'1px solid #e8ede8', borderRadius:12, padding:'14px 16px' }}>
                      ...
                      {p.abstract_snippet && (
                        <p style={{ margin:'0 0 8px', fontSize:'.77rem', color:'#6b7280', lineHeight:1.55, display:'-webkit-box' as any, WebkitLineClamp:3, WebkitBoxOrient:'vertical' as any, overflow:'hidden' }}>
                          {p.abstract_snippet}
                        </p>
                      )}
                  ...
                  {/* FIX 6: Download note — clear manual instruction */}
                  <div style={{ background:'#fffbeb', border:'1.5px solid #fde68a', borderRadius:12, padding:'14px 16px', display:'flex', alignItems:'flex-start', gap:10, marginTop:4 }}>
                    ...
                    <div>
                      ...
                      <p style={{ margin:0, fontSize:'.78rem', color:'#78350f', lineHeight:1.65 }}>
                        Download each project using the "View Project" button above and save them to your device.
                        When you reach the <strong>Past Projects</strong> step in the spec generator, upload them manually — the AI will analyse them and use them to strengthen your specification.
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        ...
        <button onClick={() => window.location.reload()} className="g-btn-outline"
          style={{ width:'100%', justifyContent:'center', fontSize:'.86rem' }}>
          Start Over · Find a Different Topic
        </button>
      </div>

      {/* FIX 5: Note confirming what gets autofilled */}
      <p style={{ textAlign:'center', marginTop:12, fontSize:'.75rem', color:'#9ca3af', lineHeight:1.65 }}>
        Clicking the button above will take you to the spec generator with your topic, field, and degree level already filled in.
        {projects && projects.length>0 && ' The similar projects found will also be pre-loaded as reference material.'}
      </p>
    </motion.div>
  )
}
  ...
  const handleTopicSelect = async (topic: Topic) => {
    ...
    try {
      const res = await axios.post(`${API}/topics/scout`, {
        ...
        field:        formData?.field || '',
        degree_level: formData?.degree_level || '',
        project_type: formData?.project_type || 'mixed',
      }, { headers:authHeaders() })
      setScoutData(res.data)
    } catch {
      setScoutData(null) // Non-critical — chat continues without it
    }
    setStage('chat')
  }

  // FIX 3: Change Topic — go back to results grid, keep existing topics loaded
  const handleChangeTopic = () => {
    setSelectedTopic(null)
    setScoutData(null)
    setStage('results') // results grid is still in state — no reload needed
  }
        ...
        <div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              ...
              <p style={{ margin:0, fontSize:'.82rem', color:'#6b7280' }}>Profile → topics → scout → AI advisor → refine → generate spec</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <motion.div key={stage} initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} transition={{ duration:.2 }}>
          ...
          {stage==='chat' && selectedTopic && formData && (
            <ChatStage
              ...
              scoutData={scoutData}
              onFinal={handleFinal}
              onChangeTopic={handleChangeTopic}  // FIX 3
            />
          )}
          ...
          {stage==='final' && finalTopic && formData && (
            <FinalStage
              ...
              field={formData.field}
              degree={formData.degree_level}
              formData={formData}  // FIX 5: pass full form for autofill
            />
          )}
