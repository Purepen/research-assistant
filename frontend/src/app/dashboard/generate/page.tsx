  ...
  return (
    <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:32,position:'relative' }}>
      <div style={{ position:'absolute',top:16,left:'12%',right:'12%',height:2,background:'#e8ede8',zIndex:0 }}>
        <motion.div initial={{width:0}} animate={{width:`${((current-1)/(STEPS.length-1))*100}%`}} transition={{duration:.4}}
          ...
          <div key={s.n} style={{ display:'flex',flexDirection:'column',alignItems:'center',position:'relative',zIndex:1,flex:1 }}>
            <motion.div initial={false} animate={{scale:active?1.08:1}}
              style={{ width:32,height:32,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,fontSize:'.82rem',transition:'all .3s',
                background:done||active?'#16a34a':'white', border:`2px solid ${done||active?'#16a34a':'#e8ede8'}`,
                color:done||active?'white':'#9ca3af', boxShadow:active?'0 0 0 5px rgba(22,163,74,.15)':'none' }}>
              {done?<I.Check/>:s.n}
            </motion.div>
...
function ChoiceBtn({ label, sub, active, onClick, color='#16a34a', bg='#f0fdf4', border='#bbf7d0' }:
  ...
  return (
    <button onClick={onClick}
      style={{ padding:'12px 16px',borderRadius:11,border:`2px solid ${active?border:'#e8ede8'}`,background:active?bg:'white',cursor:'pointer',transition:'all .15s',textAlign:'left',width:'100%' }}
...
/* ─── Drop zone ──────────────────────────────────────────────────────────── */
function DropZone({ label, accept, file, onChange, required=false, hint }:
  { label:string; accept:string; file?:File; onChange:(f:File|undefined)=>void; required?:boolean; hint?:string }) {
  ...
  return (
    <div>
      ...
      {file ? (
        <div style={{ display:'flex',alignItems:'center',gap:10,padding:'12px 14px',background:'#f0fdf4',border:'1.5px solid #bbf7d0',borderRadius:11 }}>
          <div style={{ color:'#16a34a' }}><I.File/></div>
          <div style={{ flex:1,minWidth:0 }}>
            <p style={{ margin:0,fontSize:'.84rem',fontWeight:600,color:'#0f1f0f',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{file.name}</p>
            <p style={{ margin:0,fontSize:'.72rem',color:'#9ca3af' }}>{(file.size/1024/1024).toFixed(2)} MB</p>
          </div>
          <I.Check/>
        </div>
      ) : (
        <label htmlFor={id}>
          <div onDragEnter={()=>setDragging(true)} onDragLeave={()=>setDragging(false)} onDragOver={e=>e.preventDefault()}
            ...
            style={{ border:`2px dashed ${dragging?'#16a34a':'#d1d5db'}`,borderRadius:11,padding:'28px 20px',textAlign:'center',cursor:'pointer',transition:'all .15s',background:dragging?'#f0fdf4':'#f9fafb' }}>
            <div style={{ marginBottom:8,color:dragging?'#16a34a':'#9ca3af' }}><I.Upload/></div>
            <p style={{ margin:0,fontWeight:600,color:'#374151',fontSize:'.84rem' }}><span style={{ color:'#16a34a',textDecoration:'underline' }}>Choose file</span> or drag here</p>
            {hint&&<p style={{ margin:'4px 0 0',fontSize:'.73rem',color:'#9ca3af' }}>{hint}</p>}
...
/* ─── Slider ─────────────────────────────────────────────────────────────── */
function Slider({ label, min, max, value, onChange, hint }:
  { label:string; min:number; max:number; value:number; onChange:(n:number)=>void; hint?:string }) {
  return (
    <div>
...
/* ─── Topic Lab auto-fill banner ─────────────────────────────────────────── */
function TopicLabBanner({ topic, field, level, onClear }:
  { topic:string; field:string; level:string; onClear:()=>void }) {
  return (
    <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}}
      style={{ display:'flex',alignItems:'flex-start',gap:12,background:'#faf5ff',border:'1.5px solid #e9d5ff',borderRadius:14,padding:'14px 16px',marginBottom:20 }}>
      <div style={{ width:36,height:36,borderRadius:10,background:'#ede9fe',display:'flex',alignItems:'center',justifyContent:'center',color:'#7c3aed',flexShrink:0 }}><I.Flask/></div>
      <div style={{ flex:1,minWidth:0 }}>
        <p style={{ margin:'0 0 3px',fontSize:'.78rem',fontWeight:700,color:'#7c3aed',textTransform:'uppercase',letterSpacing:'.07em' }}>Pre-filled from Topic Lab</p>
        <p style={{ margin:'0 0 6px',fontWeight:700,color:'#0f1f0f',fontSize:'.9rem',lineHeight:1.3 }}>{topic}</p>
        <p style={{ margin:0,fontSize:'.76rem',color:'#6b7280' }}>{field} · {level}</p>
      </div>
      <button onClick={onClear}
        style={{ background:'none',border:'none',cursor:'pointer',color:'#a78bfa',padding:4,display:'flex',flexShrink:0 }}
        title="Clear pre-fill">
        <I.X/>
      </button>
    </motion.div>
  )
...
function GeneratePageInner() {
  ...
  // ── Read URL params from Topic Lab hand-off ───────────────────────────────
  ...
  const fromTopics    = searchParams.get('from_topics') === 'true'

  const [step, setStep]     = useState(1)
  const [error, setError]   = useState<string|null>(null)
  const [fromTopicsBanner, setFromTopicsBanner] = useState(fromTopics)

  const [formData, setFormData] = useState({
    field_of_study:     paramField,
    research_topic:     paramTopic,
    academic_level:     (paramLevel as 'BSc'|'MSc'|'PhD') || 'MSc',
  ...
  const handleSubmit=async()=>{
    ...
    try {
      ...
      // Auto-link to topic session if came from Topic Lab
      if(paramSessionId && result.project_id) {
        try {
          await topicsApi.linkProject(parseInt(paramSessionId), result.project_id)
        } catch(linkErr) {
          console.warn('Topic link failed (non-fatal):', linkErr)
        }
      }
  ...
  return (
    <div style={{ maxWidth:780,margin:'0 auto' }}>
      <style>{`@keyframes sp2{to{transform:rotate(360deg)}} .spin2{animation:sp2 .9s linear infinite;display:inline-block} .g-input{width:100%;padding:10px 13px;border-radius:10px;border:1.5px solid #e8ede8;font-size:.87rem;color:#0f1f0f;outline:none;transition:all .2s;background:white;box-sizing:border-box} .g-input:focus{border-color:#16a34a;box-shadow:0 0 0 3px rgba(22,163,74,.1)}`}</style>

      {/* Header */}
      <div style={{ marginBottom:32 }}>
        <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:6 }}>
          <button onClick={()=>router.push('/dashboard')}
            style={{ display:'inline-flex',alignItems:'center',gap:5,background:'none',border:'none',color:'#9ca3af',cursor:'pointer',fontSize:'.8rem',fontWeight:600,padding:0,transition:'color .15s' }}
      ...
      {error&&(
        <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}}
          ...
          <p style={{ flex:1,margin:0,fontSize:'.84rem',color:'#dc2626' }}>{error}</p>
          <button onClick={()=>setError(null)} style={{ background:'none',border:'none',cursor:'pointer',color:'#9ca3af' }}><I.X/></button>
        </motion.div>
      )}

      {/* Card */}
      <div style={{ background:'white',border:'1.5px solid #e8ede8',borderRadius:16,padding:'28px',boxShadow:'0 2px 8px rgba(0,0,0,.04)' }}>
          ...
          <motion.div key={step} initial={{opacity:0,x:16}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-16}} transition={{duration:.22}}>
            ...
            {step===1&&(
              <div style={{ display:'flex',flexDirection:'column',gap:20 }}>
                <div>
                  ...
                  <p style={{ margin:0,fontSize:'.84rem',color:'#9ca3af' }}>Tell us about your research project.</p>
                </div>

                {/* Topic Lab auto-fill banner */}
                {fromTopicsBanner&&paramTopic&&(
                  <TopicLabBanner
                    topic={formData.research_topic||paramTopic}
                    field={formData.field_of_study||paramField}
                    level={formData.academic_level||paramLevel}
                    onClear={clearTopicFill}/>
                )}

                <div>
                  <label style={{ display:'block',fontSize:'.84rem',fontWeight:700,color:'#374151',marginBottom:4 }}>
                ...
                <div>
                  <label style={{ display:'block',fontSize:'.84rem',fontWeight:700,color:'#374151',marginBottom:4 }}>
                    Research Topic <span style={{ color:'#9ca3af',fontWeight:400 }}>(optional)</span>
                  </label>
                  <p style={{ margin:'0 0 7px',fontSize:'.76rem',color:'#9ca3af' }}>
                    {fromTopicsBanner&&paramTopic
                      ? 'Pre-filled from Topic Lab — edit freely.'
                      : 'Leave blank and AI will suggest a topic based on your field.'}
                  </p>
                ...
                <div>
                  <label style={{ display:'block',fontSize:'.84rem',fontWeight:700,color:'#374151',marginBottom:10 }}>
                    Academic Level <span style={{ color:'#dc2626' }}>*</span>
                  </label>
                ...
                <div>
                  ...
                  <p style={{ margin:'5px 0 0',fontSize:'.73rem',color:'#9ca3af' }}>We'll email you when your spec is ready.</p>
                </div>
              </div>
            )}
            ...
            {step===2&&(
              <div style={{ display:'flex',flexDirection:'column',gap:24 }}>
                <div>
                  <h2 style={{ margin:'0 0 4px',fontWeight:800,color:'#0f1f0f',fontSize:'1.15rem' }}>Upload Files</h2>
                  <p style={{ margin:0,fontSize:'.84rem',color:'#9ca3af' }}>Your guidelines are required. Past projects are optional but improve quality.</p>
                </div>
                <DropZone label="University Guidelines" accept=".pdf,.doc,.docx" required
                  file={files.guidelines} hint="PDF or DOCX — your department's project brief or marking rubric"
                  onChange={f=>setFiles(p=>({...p,guidelines:f}))}/>
                <MultiDropZone label="Past Projects (optional)"
                  files={files.pastProjects} onChange={f=>setFiles(p=>({...p,pastProjects:f}))}/>
                {(files.guidelines||files.pastProjects.length>0)&&(
                  <div style={{ padding:'12px 14px',background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:10,fontSize:'.8rem',color:'#374151',lineHeight:1.6 }}>
                    <strong>Pro tip:</strong> Specs with uploaded guidelines score 15–20 points higher on average. Past project examples help the AI learn your university's style.
                  </div>
                )}
              </div>
            )}
            ...
            {step===3&&(
              <div style={{ display:'flex',flexDirection:'column',gap:24 }}>
                <div>
                  <h2 style={{ margin:'0 0 4px',fontWeight:800,color:'#0f1f0f',fontSize:'1.15rem' }}>Configuration</h2>
                  <p style={{ margin:0,fontSize:'.84rem',color:'#9ca3af' }}>Advanced settings — defaults are optimised for most projects.</p>
                </div>
                <div>
                ...
                <div style={{ padding:'14px',background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:11,display:'flex',alignItems:'flex-start',gap:10 }}>
                  <div style={{ color:'#2563eb',flexShrink:0,marginTop:1 }}><I.Info/></div>
            ...
            {step===4&&(
              <div style={{ display:'flex',flexDirection:'column',gap:16 }}>
                <div>
                  <h2 style={{ margin:'0 0 4px',fontWeight:800,color:'#0f1f0f',fontSize:'1.15rem' }}>Review & Confirm</h2>
                  <p style={{ margin:0,fontSize:'.84rem',color:'#9ca3af' }}>Double-check everything before generating.</p>
                </div>

                {/* Topic Lab origin badge */}
                {fromTopicsBanner&&paramTopic&&(
                  <div style={{ display:'flex',alignItems:'center',gap:8,padding:'8px 12px',background:'#faf5ff',border:'1px solid #e9d5ff',borderRadius:10 }}>
                    <I.Flask/>
                    <span style={{ fontSize:'.78rem',color:'#7c3aed',fontWeight:600 }}>Topic sourced from Topic Lab</span>
                    {paramSessionId&&<span style={{ fontSize:'.72rem',color:'#a78bfa' }}>· Will be auto-linked to this spec</span>}
                  </div>
                )}
                ...
                <div style={{ background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:12,padding:'16px',display:'flex',alignItems:'center',gap:14 }}>
                  <div style={{ width:42,height:42,borderRadius:11,background:'#dcfce7',display:'flex',alignItems:'center',justifyContent:'center',color:'#16a34a',flexShrink:0 }}><I.Star/></div>
                  <div>
      ...
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:20 }}>
        <button className="g-btn-outline" onClick={()=>setStep(s=>s-1)} disabled={step===1||isPending} style={{ opacity:step===1?.4:1 }}>
          <I.Back/> Back
        </button>
        <div style={{ display:'flex',gap:8,alignItems:'center' }}>
          <span style={{ fontSize:'.78rem',color:'#9ca3af' }}>Step {step} of {STEPS.length}</span>
          {step<STEPS.length?(
            <button className="g-btn" onClick={()=>setStep(s=>s+1)} disabled={!canProceed()} style={{ opacity:canProceed()?1:.45 }}>
              Continue →
            </button>
          ):(
            <button className="g-btn" onClick={handleSubmit} disabled={isPending} style={{ minWidth:160,justifyContent:'center' }}>
...
/* ════════════════════════════════════════════════════════════════════════════
   EXPORT — Suspense wraps the searchParams consumer
════════════════════════════════════════════════════════════════════════════ */
export default function GeneratePage() {
  return (
    <Suspense fallback={
      <div style={{ maxWidth:780,margin:'0 auto' }}>
        <div className="g-sk" style={{ height:60,borderRadius:14,marginBottom:24 }}/>
        <div className="g-sk" style={{ height:400,borderRadius:16 }}/>
      </div>
    }>
      <GeneratePageInner/>
    </Suspense>
