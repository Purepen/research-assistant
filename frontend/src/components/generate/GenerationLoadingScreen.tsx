...
const PHASE_MAP: Record<string, { label: string; detail: string; index: number }> = {
  ...
  'Professor review complete':            { label: 'Review done',               detail: 'Packaging your completed specification…',                   index: 11 },
}
...
export function GenerationLoadingScreen({ projectId, onComplete, onError }: GenerationLoadingScreenProps) {
  ...
  const STAGE_LABELS = [
    ...
    'Past Projects', 'Synthesis', 'Requirements', 'Writing', 'Revision', 'Review', 'Done'
  ]
  ...
  return (
    <div style={{ maxWidth: 600, margin: '40px auto', padding: '0 20px 80px' }}>
      ...
      <p style={{ textAlign: 'center', marginTop: 18, fontSize: '.73rem', color: '#9ca3af' }}>
        Generation typically takes 3–8 minutes. You can safely close this tab — we'll email your spec when it's ready.
      </p>
    </div>
  )
}
