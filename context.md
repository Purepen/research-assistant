...
User has approved following `roadmap.md` in order (not a fast/simple single-container shortcut) — i.e., Workstream 1 (cloud-ready refactor: S3 state contract, Lambda entrypoints, SQLite→Postgres) before Workstream 2 (Terraform + Step Functions + per-phase Lambdas + Amplify frontend). Goal stated by the user: get something real deployed and Terraform-managed (up/down on demand) to showcase progress, following the already-approved architecture rather than a shortcut. Status: not yet started as of this entry — see roadmap.md for the full plan and this session's conversation for the decision.
